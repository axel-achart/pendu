...
def input_letter_box():
    input_box = pygame.Rect(100, 100, 50, 50)  # Position de la zone de texte pour entrer une lettre
    color_inactive = pygame.Color('lightskyblue3')
    color_active = pygame.Color('dodgerblue2')
    color = color_inactive
    active = False
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if active:
                    if event.key == pygame.K_RETURN:  # When the user hits Enter
                        return text.lower()  # Return the letter typed
                    elif event.key == pygame.K_BACKSPACE:
                        text = text[:-1]  # Delete last character
                    else:
                        if len(text) < 1:  # Only allow one letter at a time
                            text += event.unicode  # Add typed letter

        screen.fill(white)  # Clear the screen
        display_title()  # Display the game title
...
def get_letter_input(count, word_guess):
    ...
    # Créer une surface d'arrière-plan persistante si nécessaire
    background = pygame.Surface(screen.get_size())
    background.fill(white)  # Fond de l'écran
    ...
    while True:
        ...
        # Réutiliser l'arrière-plan pour éviter de redessiner l'écran
        ...
        # Dessiner la zone de saisie de la lettre
        txt_surface = font.render(text, True, black)
        input_box.w = max(50, txt_surface.get_width() + 10)
        screen.blit(txt_surface, (input_box.x + 5, input_box.y + 5))
        pygame.draw.rect(screen, color, input_box, 2)
...
def main():
    ...
    while running:
        ...
        if count > 0:  # While the user has chances
            ...
            letter = get_letter_input(count, word_guess)  # Get letter input from the input box
